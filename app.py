...
def load_data():
    ...
    return df
def load_per_data():
    per_df = pd.read_excel('PER_df_final.xlsx')
    return per_df
...
analysis = st.sidebar.selectbox('옵션을 선택하세요',['포트폴리오 분석','산업별 자산 비중','포트폴리오 위험도 분석','기업별 적정 PER'])
...
elif analysis == '기업별 적정 PER':
    st.subheader('기업별 적정 PER')
    st.write('기업별 적정 PER은 다음과 같습니다.')
    fig3 = px.bar(df_pred, x='종목명', y='적정 Per')
